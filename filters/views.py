...
def filter_interests(param_dict):
    ...
    filters = ["world", "country", "region", "local", "city", "facility", "service", "rating", "sp1", "sp2", "sp3"]

    for filter_name in filters:
        if param_dict[filter_name]:
            interestList = interestList.filter(interest_filter__in=param_dict[filter_name])
...
def filter_data(request):
    ...
    filter_types = ["RATING", "WORLD", "COUNTRY", "REGION", "LOCAL", "CITY", "SP1", "SP2", "SP3", "FACILITY", "SERVICE"]
    if page_type == "CITY": filter_types = [i for i in filter_types if i not in ("WORLD", "COUNTRY", "REGION", "LOCAL")]
    elif page_type == "LOCAL": filter_types = [i for i in filter_types if i not in ("WORLD", "COUNTRY", "REGION")]
    elif page_type == "REGION": filter_types = [i for i in filter_types if i not in ("WORLD", "COUNTRY")]
    elif page_type == "COUNTRY": filter_types = [i for i in filter_types if i not in ("WORLD")]
    all_filters = Filter.objects.filter(types__in=filter_types).prefetch_related(
        Prefetch('interests', queryset=Interest.objects.filter(display=True))
    ).order_by('order', 'name')

    filter_dict = {filter_type: all_filters.filter(types=filter_type) for filter_type in filter_types}

    count_list = []
    for filter_type, filters in filter_dict.items():
        for f in filters:
            filtered_interests = [interest for interest in f.interests.all() if interest in interests_qs]
